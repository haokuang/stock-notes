...
| 顺序 | 任务 | 优先级 | 状态 | 验收标准 |
|---|---|---|---|---|
| 1 | 修复后端编译 | P0 | 🚧 | `pnpm build:server` 成功，根目录 `pnpm build` 不再因服务端 TypeScript 错误中断 |
| 2 | 修复 Git 断链 | P0 | ✅ 2026-06-15 | 生产代码引用的 helper 与专项测试全部纳入 Git；全新检出后 `pnpm test:note-editor`、`pnpm validate` 可直接运行 |
| 3 | 修复编辑笔记数据丢失 | P0 | ⏳ | 编辑已有笔记时完整回填并保留方向、入场价、目标价、止损价、标签和图片；只修改用户实际编辑的字段 |
...
| Supabase Realtime 推送 | P1 | 🚧 | | 东京节点疑似不推送事件,代码就位但未生效;降级方案:5-10s 轮询,半天 |
...
| AI 分析中心(单图 / 跨观点) | P0 | ✅ | 2026-06-13 | |
| 我的(统计 + 设置 + 登出) | P0 | ✅ | 2026-06-13 | |
| 添加股票 / 笔记编辑 / 笔记详情 / AI 报告 / 单图解读 / 搜索 / heatmap 详情 | P0 | ✅ | 2026-06-13 | 7 个原有页面 |
| 首页持仓实时小红点(Realtime 推送) | P1 | 🚧 | | 现在只能下拉刷新,需订阅 `stocks` 表 UPDATE 事件,半天 |
| 简评证据列表弹窗 | P2 | ⏳ | | 见 § 四 |
...
| 功能 | 优先级 | 状态 | 完成日期 | 详细说明 |
|---|---|---|---|---|
| 浏览器原生 Notification(详情页 red brief 弹窗) | P1 | 🚧 | | hook 写好,依赖 Realtime 推送生效 |
| Supabase Realtime 推送(已写未生效) | P1 | 🚧 | | 见 § 一 Realtime 行 |
| 首页持仓实时小红点 | P1 | 🚧 | | 见 § 五 |
