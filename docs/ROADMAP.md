...
| 12 | 自动补齐技术指标 60 个交易日 | P0 | ✅ 2026-06-15 | 生成简评前先读取数据库；不足 60 条时从 Tushare 拉取约 120 个自然日并批量 upsert，再从数据库读取最近 60 条计算；接口返回实际样本数与是否回填 |
...
| 技术指标历史自动补齐 | P0 | ✅ | 2026-06-15 | 数据库不足 60 个交易日时拉取约 120 个自然日并批量 upsert；真实验收从 4 条补到 60 条，第二次调用不再请求 Tushare |
