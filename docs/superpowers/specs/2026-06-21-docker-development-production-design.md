...
- Taro + React 前端，可构建 H5、微信小程序和抖音小程序；本次 Docker 范围仅覆盖 H5 与微信小程序。
...
3. 微信小程序可在 Docker 中一键构建，并把产物输出到宿主机。
...
- Supabase 本地化或私有化部署。
- 微信小程序自动上传或发布。
- 抖音小程序 Docker 构建、上传与发布；该能力已由用户明确取消，原有非 Docker `pnpm build:tt` 保留。
- HTTPS 证书申请与续期。
...
H5 和微信小程序可以同时使用同一套生产 NestJS API 和云端 Supabase。
...
- 单域名反向代理说明。
- 小程序公网 API 域名配置说明。
- 微信小程序构建及导入开发者工具步骤，并说明抖音 Docker 能力不在范围内。
- 日志、健康检查、重建和停止命令。
